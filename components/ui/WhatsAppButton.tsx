...
import { WHATSAPP, WHATSAPP_URL } from "@/content/contact";
...
export function WhatsAppButton() {
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.96 }}
      className="fixed bottom-5 right-5 z-40 inline-flex items-center gap-2 rounded-full bg-[#25D366] py-3 pl-3 pr-4 text-flamingo-obsidian shadow-[0_10px_30px_-8px_rgba(37,211,102,0.55)] sm:bottom-7 sm:right-7"
    >
      <span className="relative flex h-9 w-9 items-center justify-center">
        ...
        />
        <span className="relative flex h-9 w-9 items-center justify-center rounded-full bg-white/95">
          <WhatsAppGlyph className="h-5 w-5 text-[#25D366]" />
        </span>
      </span>
      <span className="display text-xs font-bold uppercase tracking-ultra">
        {WHATSAPP.label}
      </span>
    </motion.a>
  );
